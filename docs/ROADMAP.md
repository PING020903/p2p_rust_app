...
- **0.18.0** 帧分发内核化：`Frame.text` 统一为字符串标签（`/frame/1.0.0`），`SignalRegistry`
  ...
  L3 只注册 handler/钩子
- **0.19.0** 文件传输（`/send` 事件驱动分块 + 逐块 CRC32 + 落盘校验）+ 可配置下载目录 +
  IPv6 直连地址 UX（启动打印 / `/listen` 重打）；`docs/CROSS_LAN_CONNECTIVITY.md`（跨局域网方案）
- **0.20.0** 对称信任（L2 互信门控，任一方取消整链断）+ cmd//ps/ 终端逃逸 +
  测试拆分（逻辑默认 / 稳定性 `--ignored` 显式）
- **0.21.0** 严格 L3→L2→L1 分层：seam 传输适配层（L3 只见 tag+payload）+ L1 `pub(crate)`
  内部化 + hello/bye/trust 内化为 L2 `TextTag` + 未互信信号钩子；`docs/PROJECT_ARCHITECTURE.md`
...
- **已实现**：`/send <角色|节点ID> <路径>`（须互信）——事件驱动分块（1 MiB）推送 +
  逐块 CRC32 校验 + 接收方落盘校验；停等 ack 天然背压
- 复用身份验证 + 联系人簿：发送前校验接收方是**已验证联系人**（"发给谁"由传输层保证）
- 下载目录默认用户 Downloads，`/download-dir <路径>` 可改并持久化
...
- **群聊后续**：群消息历史持久化；重连时向群主请求名单快照（补偿漏收）；
  **名单权威性加固**——当前 `GroupMemberList`/`GroupOwnerTransfer` 无发送者权威校验
  （仅 version 门控），手动修改本地 `groups_<id>.json` 的群主/名单可被改者单向污染
  组内名单（能加人/声明转移，但无法冒充他人发言——群消息是 Ed25519 签名）。
  加固方向（方案 2，未来）：群主用 Ed25519 私钥对 `(version, members)` 签名，帧内携带，
  接收方用群主公钥（可从 PeerId 提取）验签；配合 `/chat/8.0.0` 与 CRDT 路线图
...
- **跨城市 IPv6 实测（待办）**：IPv6 直连地址互拨已实现（0.19.0）但**未经真实跨城市双端验证**——
  需一位跨城市测试伙伴配合（双方全局 IPv6 + 路由器放行端口，互 `/dial` 收发消息/文件）；
  验证通过后更新 `docs/CROSS_LAN_CONNECTIVITY.md` 状态为"已验证"
