...
- **P2.6 步3：UntrustedSend Ask + 接收拦截提示（信任语义可见化）**：
  - **发送侧**（D3 三态补全）：未互信发送在 Ask 模式（GUI）弹 UntrustedSend 卡片
    （"对方 {名} 未互信，消息可能被对端忽略" + [仍要发送]/[取消发送]）——确认后发送并记忆
    （send_confirmed，对齐 CLI D3 首次确认语义）；Auto（e2e）保持放行但文案如实修正：
    "消息已发出（对端未互信时可能被忽略）"（此前"消息仍已发送"严重误导——
    日志实证：14:24:43/45 双方各自"已发送"，双向门禁各自丢弃，互没收到）
  - **接收侧**：未互信来源业务信号被门禁丢弃时，时间线出黄字拦截提示
    （"收到未信任方 {名} 的业务消息已拦截（互信后可见）: {tag}"，每条都提示）——
    单方面信任的"不通"从零痕迹变为可诊断
