//! /dial 地址域：拨号地址解析（严格校验）与用法模板。
//! GUI"添加联系人"表单与 CLI /dial 命令共用同一校验（Control::Dial → 本模块）。

use colored::Colorize;
use libp2p::{Multiaddr, PeerId};

pub(crate) fn print_dial_template() {
    println!("{}", "地址格式:".yellow());
...
/// 解析拨号地址：容忍"监听地址:"前缀（GUI 复制整行直接粘），逐段校验后整体转 Multiaddr
pub(crate) fn parse_dial_addr(input: &str) -> Result<Multiaddr, String> {
