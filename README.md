...
1. 两个终端分别 `cargo run`，都选 `4`，完成**角色登录**（姓名 / 生日 YYYY-MM-DD / 性别 / 密码，密码不回显）
   - 同样的登录信息 → 同一个角色 ID：跨重启、跨机器节点 ID 都不变
...
| P2P 聊天（libp2p） | ✅ 局域网可用 | 角色登录（SHA-256 盐 + Argon2id + Ed25519 确定性身份）；TCP + Noise 加密 + Yamux 多路复用；mDNS 发现登记 + /chat 按需 1v1；心跳包活 + 超时判离线；Hello/Bye 上下线通知；断线自动重连 |
...
| M7 | 多用户角色身份与按需 1v1 | ✅ 登录派生身份 + Hello 花名册 + /list + /chat 按名呼叫 |
...
- 端到端集成测试 1 项，含 4 个串行场景：基础聊天（登录/带名字的上线通知/双向收发/静默 12 秒心跳保活/优雅退出）、按角色名呼叫、主动上下线循环 ×15、kill 掉线循环 ×5（每轮发送 ≤64 字节随机消息）
