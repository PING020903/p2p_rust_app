...
7. **发现模式**（隐私）：`/discover advertise|stealth|off` 切换并持久化，下次进入聊天生效
...
- 端到端集成测试 1 项，含 7 个串行场景：基础聊天（登录/带名字的上线通知/双向收发/静默 12 秒心跳保活/优雅退出）、按角色名呼叫、主动上下线循环 ×15、kill 掉线循环 ×5（每轮发送 ≤64 字节随机消息）、身份缓存回环（错密码校验）、同 ID 冲突拒绝、隐身发现模式（只收不发）
