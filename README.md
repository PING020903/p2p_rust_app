...
9. **群聊**（微信式）：`/group new <群名>` 建群 → `/group add <群名> <角色>` 拉人（须已验证联系人，对方收到邀请自动入群）→ `/group <群名>` 聚焦后输入即发群里
   ...
   - **群成员以群主为中心**：仅群主可邀请；成员退群（`/group leave <群名>`）通知群主划去；
     每次成员变更，群主向最新名单所有成员 1v1 扇出更新（版本化整体替换）
   - **常驻接收**（防通讯风暴）：`/group resident <群名> on|off` —— 常驻群成员上线自动连接维持接收；
     普通群（默认）只在聚焦时按需连接、聚焦时才收发
   - `/group list` 查看群、成员数、名单版本与 `[常驻]` 标记
...
>
> 1v1 消息帧分三通道：`control`（心跳/传输控制）、`text`（节点间短消息，非用户内容）、
> `binary`（用户内容负载：聊天文本、群管理，cbor 自描述）。群文本走 gossipsub。
