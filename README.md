...
| P2P 聊天（libp2p） | ✅ 局域网可用 | 身份=随机种子+12 词 BIP39 助记词派生（密码仅护本地加密 keystore，姓名/生日/性别为资料元数据）；TCP + Noise 加密 + Yamux 多路复用；mDNS 发现登记 + /chat 按需 1v1（多会话，微信式切换焦点）；心跳包活 + 超时判离线；Hello/Bye 上下线通知；断线自动重连（队列）；/backup 重看助记词；TOFU 指纹核对 + 联系人簿 + /trust；发现模式（广播/隐身/关闭） |
...
- 端到端集成测试 1 项，含 8 个串行场景：基础聊天（登录/带名字的上线通知/双向收发/静默 12 秒心跳保活/优雅退出）、按角色名呼叫、主动上下线循环 ×15、kill 掉线循环 ×5（每轮发送 ≤64 字节随机消息）、身份缓存回环（错密码校验）、同 ID 冲突拒绝、隐身发现模式（只收不发）、三节点 1v1 多会话（并发连接/焦点切换/非焦点来信带名）
