...
4. 连上后直接打字就是聊天消息（发给当前聊天对象）；`/help` 查看命令；`/quit` 退出（会向对方发送下线通知）
5. `/backup` 可随时重新查看本身份助记词（需输入密码）
6. 首次连接新节点时，会展示对方**身份指纹**（SSH 式短指纹 + 节点 ID）要求确认，确认后记录为联系人；`/list` 显示信任状态，`/trust` / `/trust !` 可随时标记/取消信任
...
| P2P 聊天（libp2p） | ✅ 局域网可用 | 身份=随机种子+12 词 BIP39 助记词派生（密码仅护本地加密 keystore，姓名/生日/性别为资料元数据）；TCP + Noise 加密 + Yamux 多路复用；mDNS 发现登记 + /chat 按需 1v1；心跳包活 + 超时判离线；Hello/Bye 上下线通知；断线自动重连；/backup 重看助记词；TOFU 指纹核对 + 联系人簿 + /trust |
...
- 单元测试 28 项（指令树 8 + 地址诊断 10 + 身份/keystore 6 + 资料归一 2 + 指纹/联系人簿 2）
- 端到端集成测试 1 项，含 6 个串行场景：基础聊天（登录/带名字的上线通知/双向收发/静默 12 秒心跳保活/优雅退出）、按角色名呼叫、主动上下线循环 ×15、kill 掉线循环 ×5（每轮发送 ≤64 字节随机消息）、身份缓存回环（错密码校验）、同 ID 冲突拒绝
- e2e 登录凭据从 `tests/users.txt` 读取（首次运行前复制 `tests/users.template.txt` 创建，该文件不入库）；测试身份用 BIP39 官方测试向量助记词（代码内常量，仅测试用）
